alter table public.catalog_garment_types
  add column if not exists body_measurement_layout text not null default 'columns';

alter table public.catalog_garment_types
  drop constraint if exists catalog_garment_types_body_measurement_layout_check;

alter table public.catalog_garment_types
  add constraint catalog_garment_types_body_measurement_layout_check
  check (body_measurement_layout in ('columns', 'compact_legacy'));

update public.catalog_garment_types
set body_measurement_layout = 'compact_legacy'
where lower(btrim(name)) in ('chudidar', 'blouse');
...
begin
  ...
  if p_garment_type_id is null then
    insert into public.catalog_garment_types (
      ...
      addon_ids, show_order_addons, body_measurement_layout, is_active
    ) values (
      ...
      p_body_measurement_layout, p_is_active
