-- Phase 2: atomically saves a garment's basic catalog record and its dynamic
-- field mappings. The function is the only writer used by the new admin UI.

create or replace function save_garment_type_configuration(
...
declare
  v_garment_id uuid;
begin
  if not auth_has_permission('catalog.manage') then
  ...
  if exists (
    select 1
    from jsonb_array_elements(coalesce(p_field_assignments, '[]'::jsonb)) assignment
    left join catalog_fields field on field.id = nullif(assignment->>'fieldId', '')::uuid
  ...
  if exists (
    ...
    left join catalog_addons addon on addon.id = addon_id
  ...
  delete from garment_type_fields where garment_type_id = v_garment_id;
...
grant execute on function save_garment_type_configuration(
