-- Allow garment body measurements to use a compact paper-like row layout.
-- This is only metadata/configuration; saved orders and measurements are not changed.
...
alter table public.catalog_garment_types
  ...
  check (body_measurement_layout in ('columns', 'compact_legacy', 'paper_rows'));
...
begin
  ...
  if p_body_measurement_layout not in ('columns', 'compact_legacy', 'paper_rows') then
