...
export function ProductionQueue({
  ...
  return (
    <section
      className={cn(
        "flex flex-col rounded-xl border border-border-soft bg-white shadow-soft",
        className
      )}
    >
      <div className="flex items-start justify-between gap-3 border-b border-border-soft px-5 py-4">
        <div>
          ...
          <p className="mt-1 text-[13px] text-ink-faint">
            Pending job cards by current stage
          </p>
        </div>
      ...
      {stages.length === 0 ? (
        <div className="px-5 py-5 text-sm text-ink-muted">
          No pending job cards. Production is clear.
        </div>
      ) : (
        <div className="flex-1 divide-y divide-border-soft">
          {stages.map((stage) => (
            <Link
              key={stage.stage}
              href="/job-cards"
              className="flex min-h-[58px] items-center gap-3 px-5 py-3 transition duration-200 hover:bg-surface-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary"
            >
              ...
              <span className="min-w-7 text-right text-lg font-bold text-primary">
                {stage.count}
              </span>
